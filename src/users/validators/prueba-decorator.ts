...
export const ValidatePass = createParamDecorator(
    async (data: unknown, ctx: ExecutionContext) => {

        //context (ctx) captura la solicitud, y luego se puede extraer el cuerpo.
        const request = ctx.switchToHttp().getRequest()
        const user: CreateUserDto =  await request.body
    

        if(!user.nombre) throw new HttpException('Debes ingresar el nombre', HttpStatus.BAD_REQUEST);
        if(!user.nombre) throw new HttpException('Debes ingresar el nombre', HttpStatus.BAD_REQUEST);
        if(!user.nombre) throw new HttpException('Debes ingresar el nombre', HttpStatus.BAD_REQUEST);
